...
from solarpanel_forecaster.utils.soliscloud_api import SoliscloudAPI


class Helpers():

    @staticmethod
    async def get_station_ids(api: SoliscloudAPI, key, secret, nmi=None) -> tuple(int):
        """ Parses response from get_station_list and returns all station id's """
        response = await api.user_station_list(key, secret, page_no=1, page_size=100, nmi_code=nmi)
        stations = ()
    ...
    async def get_inverter_ids(api: SoliscloudAPI, key, secret, station_id: int = None, nmi=None) -> tuple(int):
        ...
        else all inverter id's for all stations
        """
        response = await api.inverter_list(key, secret, page_no=1, page_size=100, station_id=station_id, nmi_code=nmi)
        inverters = ()
        for element in response:
